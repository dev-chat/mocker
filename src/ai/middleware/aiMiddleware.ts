...
export const aiMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!hasAvailableMoonToken && isAlreadyAtMaxRequests) {
    aiMiddlewareLogger.info(
      `User ${user_id} from team ${team_id} has reached max requests without a Moon Token.`,
    );
    res.send(
    ...
    aiMiddlewareLogger.info(
      `User ${user_id} from team ${team_id} has an in-flight request.`,
    );
    ...
    aiMiddlewareLogger.info(
      `User ${user_id} from team ${team_id} has reached max requests but has a Moon Token.`,
    );
