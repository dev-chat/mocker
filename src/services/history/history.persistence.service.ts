...
export class HistoryPersistenceService {
  ...
  async getHistory(request: SlashCommandRequest): Promise<MessageWithName[]> {
    ...
    const query = `
    select message.*, slack_user.name, message.createdAt from message INNER JOIN slack_user ON slack_user.id=message.userIdId WHERE message.createdAt >= NOW() - INTERVAL 30 MINUTE AND message.teamId=? AND message.channel=? ORDER BY createdAt DESC;`;
