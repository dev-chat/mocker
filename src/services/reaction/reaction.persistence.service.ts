...
export class ReactionPersistenceService {
  ...
  public getRepByUser(userId: string, teamId: string): Promise<ReactionByUser[]> {
    return getRepository(Reaction)
      .query(
        `SELECT reactingUser, SUM(value) as rep FROM reaction WHERE affectedUser=? AND teamId=? AND reactingUser IS NOT 'ADMIN' GROUP BY reactingUser ORDER BY rep DESC;`,
        [userId, teamId],
      )
      .then(value => value)
      .catch(e => {
