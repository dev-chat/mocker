...
describe('TranslationService', () => {
  ...
  it('selects russian for roll in [0.23, 0.25)', async () => {
    ...
    (Axios.post as Mock).mockResolvedValue({ data: { data: { translations: [{ translatedText: 'privet' }] } } });

    await service.translate('hello');

    expect((Axios.post as Mock).mock.calls[0][1].target).toBe('ru');
  });

  it('selects french for roll in [0.21, 0.23)', async () => {
    vi.spyOn(Math, 'random').mockReturnValue(0.22);
    (Axios.post as Mock).mockResolvedValue({ data: { data: { translations: [{ translatedText: 'salut' }] } } });

    await service.translate('hello');

    expect((Axios.post as Mock).mock.calls[0][1].target).toBe('fr');
  });
  ...
  it('selects japanese for roll in [0.19, 0.21)', async () => {
    ...
    expect((Axios.post as Mock).mock.calls[0][1].target).toBe('ja');
  });

  it('selects german for roll < 0.19', async () => {
    vi.spyOn(Math, 'random').mockReturnValue(0.1);
    (Axios.post as Mock).mockResolvedValue({ data: { data: { translations: [{ translatedText: 'hallo' }] } } });

    await service.translate('hello');

    expect((Axios.post as Mock).mock.calls[0][1].target).toBe('de');
  });
